...
#define EEPROM_CO2_BASELINE_VOLTAGE_TABLE (EEPROM_CO_BASELINE_VOLTAGE_TABLE - (5*sizeof(baseline_voltage_t))) // array of (up to) five structures for baseline offset characterization over temperature
...
#define EEPROM_BACKUP_CO2_SENSITIVITY    (EEPROM_BACKUP_CO_CAL_OFFSET + 4)
    ...
    if(first || ((current_millis - previous_display_update) >= display_update_interval)) {
        ...
        default:
            ...
            if(co2_ready || (sample_buffer_idx > 0)) {
                updateLCD(co2_ppm, 13, 1, 3, false);
            }
            else {
...
void initializeHardware(void) {
    ...
    Serial.println(F(" |     CO2 / PM / CO Sensor Suite     |"));
...
void restore(char * arg) {
    ...
    if (strncmp(arg, "defaults", 8) == 0) {
        ...
        configInject(F("sampling 5, 480, 60\r")); // sample every 5 seconds, average over 8 minutes, report every minute
        ...
        eeprom_write_block("aqev2_co2_pm_co_esp", (void *) EEPROM_UPDATE_FILENAME, 32);
    }
...
boolean publishHeartbeat() {
    ...
    snprintf(scratch, SCRATCH_BUFFER_SIZE-1,
             ...
             "\"converted-value\":%d,"
             "\"firmware-version\":\"%s\","
             "\"publishes\":[\"co2\",\"particulate\",\"co\",\"temperature\",\"humidity\"%s],"
             "\"counter\":%lu"
